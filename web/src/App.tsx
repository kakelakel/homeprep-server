...
type PlanCheckEditor = { plan: Plan; item: PlanCheck | null } | null;
...
function App() {
  ...
  const [planCheckEditor, setPlanCheckEditor] = useState<PlanCheckEditor>(null);
  ...
  async function savePlan(event: FormEvent<HTMLFormElement>, editor: NonNullable<Editor<Plan>>) {
    ...
    const form = new FormData(event.currentTarget);
    try {
      const body = { name: String(form.get("name") ?? "").trim(), plan_type: String(form.get("plan_type") ?? "other"), meeting_point: String(form.get("meeting_point") ?? "").trim() || null, review_interval_months: Number(form.get("review_interval_months") ?? 0), description: String(form.get("description") ?? "").trim() || null, enabled: form.get("enabled") === "on", notes: String(form.get("notes") ?? "").trim() || null };
  ...
  async function savePlanCheck(event: FormEvent<HTMLFormElement>, editor: NonNullable<PlanCheckEditor>) {
    event.preventDefault();
    const form = new FormData(event.currentTarget);
    const payload = {
      expected_revision: editor.plan.revision,
      label: String(form.get("label") ?? "").trim(),
      description: String(form.get("description") ?? "").trim() || null,
      linked_inventory_item_ids: form.getAll("linked_inventory_item_ids").map(String),
      linked_container_ids: form.getAll("linked_container_ids").map(String),
      linked_asset_ids: form.getAll("linked_asset_ids").map(String),
    };
    try {
      if (editor.item) {
        await request(`/api/v1/plans/${editor.plan.id}/checklist/${editor.item.id}`, { method: "PATCH", body: JSON.stringify(payload) });
      } else {
        await request(`/api/v1/plans/${editor.plan.id}/checklist`, { method: "POST", body: JSON.stringify(payload) });
      }
      setPlanCheckEditor(null);
      await refresh();
    } catch (err) { fail(err, "Unable to save checklist item"); }
  }

  async function deletePlanCheck(plan: Plan, item: PlanCheck) {
    if (!window.confirm(`Delete checklist item “${item.label}”?`)) return;
    try {
      await request(`/api/v1/plans/${plan.id}/checklist/${item.id}?expected_revision=${plan.revision}`, { method: "DELETE" });
      setPlanCheckEditor(null);
      await refresh();
    } catch (err) { fail(err, "Unable to delete checklist item"); }
  }
  ...
  const overview = useMemo(() => {
    ...
    const containerAttention = data.containers.filter((container) => Boolean(container.next_check_at && dateOnly(container.next_check_at) <= today) || data.inventory.filter((item) => item.container_id === container.id).some((item) => (item.expires_at && item.expires_at < today) || (item.next_check_at && item.next_check_at <= today))).length;
    const assetDue = data.assets.filter((asset) => asset.next_check_at && dateOnly(asset.next_check_at) <= today).length;
    const planAttention = data.plans.filter((plan) => plan.enabled && (!(plan.checklist.length > 0 && plan.checklist.every((item) => item.completed)) || Boolean(plan.next_review_at && plan.next_review_at <= today))).length;
    const fallbackAreas = [
      { key: "inventory", label: "Inventory", configured: data.inventory.length > 0, score: data.inventory.length ? Math.round(100 * (data.inventory.length - inventoryAttention) / data.inventory.length) : null, detail: `${inventoryAttention} need attention`, tracked: data.inventory.length },
      { key: "containers", label: "Containers", configured: data.containers.length > 0, score: data.containers.length ? Math.round(100 * (data.containers.length - containerAttention) / data.containers.length) : null, detail: `${containerAttention} need attention`, tracked: data.containers.length },
      { key: "tasks", label: "Tasks", configured: data.tasks.some((task) => task.enabled), score: null, detail: `${data.tasks.filter((task) => task.enabled && ["overdue", "due", "upcoming", "unscheduled"].includes(taskUiStatus(task))).length} due / upcoming`, tracked: data.tasks.filter((task) => task.enabled).length },
      { key: "assets", label: "Assets", configured: data.assets.length > 0, score: data.assets.length ? Math.round(100 * (data.assets.length - assetDue) / data.assets.length) : null, detail: `${assetDue} checks due`, tracked: data.assets.length },
      { key: "plans", label: "Plans", configured: data.plans.some((plan) => plan.enabled), score: null, detail: `${planAttention} need attention`, tracked: data.plans.filter((plan) => plan.enabled).length },
      { key: "targets", label: "Targets", configured: Boolean(readiness?.targets.total), score: readiness?.targets.total ? Math.round(100 * readiness.targets.met / readiness.targets.total) : null, detail: `${(readiness?.targets.total ?? 0) - (readiness?.targets.met ?? 0)} not fully ready`, tracked: readiness?.targets.total ?? 0 },
    ];
    const areas = readiness?.areas?.length ? readiness.areas.map((area) => ({ ...area, tracked: area.tracked })) : fallbackAreas;
    const configuredScores = fallbackAreas.filter((area) => area.configured && area.score !== null).map((area) => area.score as number);
    const fallbackOverall = configuredScores.length ? Math.round(configuredScores.reduce((sum, score) => sum + score, 0) / configuredScores.length) : null;
    return { areas, overall: readiness?.score ?? fallbackOverall, inventoryAttention, containerAttention, assetDue, planAttention, pendingShopping: data.shopping.filter((item) => item.status === "pending").length };
  }, [data, readiness]);
  ...
  const overallValue = overview.overall;
  const overviewTone = overallValue === null ? "attention" : overallValue >= 75 ? "ready" : overallValue >= 50 ? "attention" : "critical";
  const overviewTitle = overallValue === null ? "Configure your preparedness" : overallValue >= 75 ? "Home is ready" : overallValue >= 50 ? "Requires attention" : "Action required";
  ...
  return <main className="shell">
    ...
    {page === "overview" && <section className="content-width overview-page">
      <div className={`overview-banner hp-${overviewTone}`}><div className="banner-icon">⌂</div><div><h2>{overviewTitle}</h2><p>Readiness across supplies, storage, maintenance and household plans.</p></div><div className="overall-score"><strong>{overallValue === null ? "—" : `${overallValue}%`}</strong><span>OVERALL READINESS</span></div></div>
      <section className="hp-panel readiness-panel"><h3>⌁ Readiness by area</h3><div className="area-grid">{overview.areas.map((area) => <article className={`area-card ${area.configured ? "" : "not-configured"}`} key={area.key}><div className="area-head"><strong>{area.label}</strong><b>{area.score === null ? "—" : `${area.score}%`}</b></div><div className="progress"><span style={{ width: `${area.score ?? 0}%` }} /></div><div className="area-meta"><span>{area.configured ? area.detail : "Not configured"}</span><span>{area.configured ? `${area.tracked} tracked` : "Excluded from overall"}</span></div></article>)}</div></section>
    ...
    {planCheckEditor && <Modal title={planCheckEditor.item ? "Edit checklist item" : "Add checklist item"} onClose={() => setPlanCheckEditor(null)}><PlanChecklistForm editor={planCheckEditor} inventory={data.inventory} containers={data.containers} assets={data.assets} save={savePlanCheck} remove={deletePlanCheck} close={() => setPlanCheckEditor(null)} /></Modal>}
...
function PlanForm({ editor, save, close }: { editor: NonNullable<Editor<Plan>>; save: (event: FormEvent<HTMLFormElement>, editor: NonNullable<Editor<Plan>>) => Promise<void>; close: () => void }) {
  const item = editor.mode === "edit" ? editor.value : null;
  return <form className="edit-form" onSubmit={(event) => void save(event, editor)}><Field label="Name"><input name="name" required defaultValue={item?.name ?? ""} /></Field><div className="form-grid two"><Field label="Plan type"><select name="plan_type" defaultValue={item?.plan_type ?? "other"}>{Object.entries(PLAN_TYPES).map(([value, text]) => <option key={value} value={value}>{text}</option>)}</select></Field><Field label="Meeting point (optional)"><input name="meeting_point" defaultValue={item?.meeting_point ?? ""} /></Field><Field label="Review every (months)"><input type="number" name="review_interval_months" min={0} defaultValue={item?.review_interval_months ?? 6} /></Field><Field label="Next review"><input value={item?.next_review_at ?? "Not scheduled"} readOnly /></Field></div><Field label="Description"><textarea name="description" defaultValue={item?.description ?? ""} /></Field><label className="check-row"><input type="checkbox" name="enabled" defaultChecked={item?.enabled ?? true} />Enabled</label><p className="hint">Checklist items are managed from the plan card so their descriptions and linked Inventory, Containers and Assets are preserved individually.</p><Field label="Notes"><textarea name="notes" defaultValue={item?.notes ?? ""} /></Field><FormActions submitLabel="Save plan" onCancel={close} /></form>;
}

function PlanChecklistForm({ editor, inventory, containers, assets, save, remove, close }: { editor: NonNullable<PlanCheckEditor>; inventory: InventoryItem[]; containers: Container[]; assets: Asset[]; save: (event: FormEvent<HTMLFormElement>, editor: NonNullable<PlanCheckEditor>) => Promise<void>; remove: (plan: Plan, item: PlanCheck) => Promise<void>; close: () => void }) {
  const item = editor.item;
  return <form className="edit-form" onSubmit={(event) => void save(event, editor)}><Field label="Checklist item"><input name="label" required defaultValue={item?.label ?? ""} /></Field><Field label="Description"><textarea name="description" defaultValue={item?.description ?? ""} /></Field><div className="form-grid three"><Field label="Linked Inventory"><select multiple name="linked_inventory_item_ids" defaultValue={item?.linked_inventory_item_ids ?? []}>{inventory.map((entry) => <option key={entry.id} value={entry.id}>{entry.name}</option>)}</select></Field><Field label="Linked Containers"><select multiple name="linked_container_ids" defaultValue={item?.linked_container_ids ?? []}>{containers.map((entry) => <option key={entry.id} value={entry.id}>{entry.name}</option>)}</select></Field><Field label="Linked Assets"><select multiple name="linked_asset_ids" defaultValue={item?.linked_asset_ids ?? []}>{assets.map((entry) => <option key={entry.id} value={entry.id}>{entry.name}</option>)}</select></Field></div><p className="hint">Hold Ctrl/Cmd to select multiple linked resources. Links use the same Plan schema as Home Assistant.</p><div className="form-actions"><button type="submit">Save checklist item</button>{item && <button type="button" className="danger" onClick={() => void remove(editor.plan, item)}>Delete</button>}<button type="button" className="secondary" onClick={close}>Cancel</button></div></form>;
}
...
function ShoppingForm({ editor, taxonomy, containers, preferredUnits, save, close }: { editor: NonNullable<Editor<ShoppingItem>>; taxonomy: InventoryTaxonomy | null; containers: Container[]; preferredUnits: (category: string) => string[]; save: (event: FormEvent<HTMLFormElement>, editor: NonNullable<Editor<ShoppingItem>>) => Promise<void>; close: () => void }) {
  ...
  return <form className="edit-form" onSubmit={(event) => void save(event, editor)}><Field label="Name"><input name="name" required defaultValue={item?.name ?? ""} /></Field><div className="form-grid two"><Field label="Quantity"><input type="number" step="any" min={0} name="quantity" defaultValue={item?.quantity ?? 1} /></Field><Field label="Unit"><select name="unit" value={unit} onChange={(event) => setUnit(event.currentTarget.value)}>{preferredUnits(category).map((value) => <option key={value} value={value}>{unitLabel(value, taxonomy)}</option>)}</select></Field><Field label="Category"><select name="category" value={category} onChange={(event) => changeCategory(event.currentTarget.value)}>{Object.entries(taxonomy?.categories ?? { other: "Other" }).map(([value, text]) => <option key={value} value={value}>{text}</option>)}</select></Field><Field label="Container (optional)"><select name="container_id" defaultValue={item?.container_id ?? ""}><option value="">No container</option>{containers.map((container) => <option key={container.id} value={container.id}>{container.name}</option>)}</select></Field></div><Field label="Notes"><textarea name="notes" defaultValue={item?.notes ?? ""} /></Field><FormActions submitLabel="Save" onCancel={close} /></form>;
}
